...
class Karp:
    ...
    def calculate_c_i_j(self, n, vector_X, vector_Y):
        ...
        for i in range(n):
            for j in range(n):
                c[i, j] = np.inner(vector_X[i], vector_Y[i] - vector_Y[j])
        return c
    ...
    def calculate_c_i_j_tilda(self, c, epsilon_star):
        ...
        """
        c_tilda = np.zeros((len(self.vector_X), len(self.vector_X)))
        for i in range(len(self.vector_X)):
    ...
    def execute_karps_algorithm(self):
        ...
        Notes
        -----
        Only tested and implemented for dimension d = 1
        Needs to be adjusted to d > 1
        """

        # Karp's function begin
        """Calculate c_{i,j}"""
        n = len(self.vector_X)
        c = self.calculate_c_i_j(n, self.vector_X, self.vector_Y)
