...
class Total_Reparation:
    ...
    def transportation_problem(self, n_0, n_1, C):
        ...
        model.writeLP(r"..\examples\data\TransportationProblem.lp")
    ...
    def define_total_repair_opcionA(self, set_0, set_1, n_0, n_1, pi_0, pi_1, gamma_opt):
        """
        Function that calculates opcion A, the repair values according to the papaer Obtaining Fairness section 4 - Option A.
        :param set_0: pandas Dataframe
        :param set_1: pandas Dataframe
        :param n_0: number of minority samples
        :param n_1: number of default samples
        ...
        repared_0 = np.zeros(set_0.shape, dtype=float)
        repared_1 = np.zeros(set_1.shape, dtype=float)

        # Reparacion X_{0}
        for i in range(n_0):
            a = 0
            for j in range(n_1):
                a += gamma_opt[i,j]*set_1.iloc[j,:]
            repared_0[i] = (pi_0 * set_0.iloc[i,:]) + (n_0*pi_1)*a

        # Reparacion X_{1}
        for j in range(n_1):
            b = 0
            for i in range(n_0):
                b += gamma_opt[i,j]*set_0.iloc[i,:]
            repared_1[j] = (pi_1 * set_1.iloc[j,:]) + (n_1*pi_0)*b

        return (repared_0, repared_1)

    def define_total_repair_opcionB(self, set_0, set_1, n_0, n_1, pi_0, pi_1, gamma_opt):
        """
        ...
        :return: tuple (np.array, all non negative values in a list)
        """
        repared = np.zeros((n_0, n_1, set_0.shape[1]), dtype=float)
        total_repared = []  # list with the positive elements
        for i in range(n_0):
            for j in range(n_1):
                if gamma_opt[i,:][0,j] > 0:
                    repared[i,j] = (pi_0 * set_0.iloc[i,:]) + (pi_1 * set_1.iloc[j,:])
    ...
    def run_transportation_problem(self, input_min, input_def, n_des, n_fav):
        ...
        cuadr_cost_matrix = self.cuadratic_cost_function(input_min, input_def, n_des, n_fav)
        savetxt(r'..\examples\data\c_transp_prob.csv', cuadr_cost_matrix, delimiter=',')

        # 6. TRANSPORTATION PROBLEM
        C = genfromtxt(r'..\examples\data\c_transp_prob.csv', delimiter=',')
        ...
        savetxt(r'..\examples\data\gamma_opt.csv', gamma_opt, delimiter=',')
        return gamma_opt
    ...
    def run_total_repair_option_A(self):
        ...
        if continues:
            try:
                ...
                # Option A
                reparated_points_A = self.define_total_repair_opcionA(self.input_0, self.input_1, n_des, n_fav, pi_0,
                                                                  pi_1, self.run_transportation_problem(self.input_0, self.input_1, n_des, n_fav))
                df0 = pd.DataFrame(reparated_points_A[0])
                df1 = pd.DataFrame(reparated_points_A[1])
                reparated_points_A_df = pd.concat([df0, df1], ignore_index=True, axis=0)
                self.reparated_points_A_array = reparated_points_A_df
                reparated_points_A_df.to_csv(r'..\examples\data\reparated_points_a.csv', index=False)
    ...
    def run_total_repair_option_B(self):
        ...
        if continues:
            ...
            try:
                ...
                pi_1 = n_fav / (n_des + n_fav)
                # Option B
                (reparated_points_B, list_reparated_points_B) = self.define_total_repair_opcionB(self.input_0, self.input_1, n_des, n_fav,
                                                               pi_0, pi_1, self.run_transportation_problem(self.input_0, self.input_1, n_des, n_fav))

                #df0 = pd.DataFrame(list_reparated_points_B)
                #df1 = pd.DataFrame(list_reparated_points_B)
                #reparated_points_B_df = pd.concat([df0, df1], ignore_index=True, axis=1)
                #self.reparated_points_B_array = reparated_points_B_df
                reparated_points_B_df = pd.DataFrame(list_reparated_points_B)
                self.reparated_points_B_array = reparated_points_B_df
                #reparated_points_B_df.to_csv(r'..\examples\data\reparated_points_b.csv', index=False)
                reparated_points_B_df.to_csv(r'..\examples\data\reparated_points_b.csv', index=False)
        ...
        try:
            if opcion == 'A':
                ...
                if group == 0:
                    ...
                    return (self.input_0, self.reparated_points_A_array.iloc[:self.input_0.shape[0]])

                elif group == 1:

                    self.run_total_repair_option_A()
                    return (self.input_1, self.reparated_points_A_array.iloc[self.input_0.shape[0]:])
                ...
                if group == 0:
                    ...
                    return (self.input_0, self.reparated_points_B_array)

                elif group == 1:

                    self.run_total_repair_option_B()
                    return (self.input_1, self.reparated_points_B_array)
